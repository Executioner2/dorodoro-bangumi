...
async fn send_get_peers(socket: &mut UdpSocket, addr: &SocketAddr, id: &NodeId, info_hash: &NodeId) -> Result<()> {
    ...
    socket.send_to(&get_peers.to_bencode().unwrap(), &addr).await?;
...
async fn send_find_node(socket: &mut UdpSocket, addr: &SocketAddr, id: &NodeId, target: &NodeId) -> Result<()> {
    ...
    socket.send_to(&find_node.to_bencode().unwrap(), &addr).await?;

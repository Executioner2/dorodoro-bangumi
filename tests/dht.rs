...
use bytes::Bytes;
use dorodoro_bangumi::bencoding::BEncodeHashMap;
use dorodoro_bangumi::bencoding::BEncoder;
...
use dorodoro_bangumi::peer::peer_resp::{PeerResp, RespType};
use dorodoro_bangumi::peer::{reserved, MsgType};
use dorodoro_bangumi::protocol::{
    BIT_TORRENT_PAYLOAD_LEN, BIT_TORRENT_PROTOCOL, BIT_TORRENT_PROTOCOL_LEN,
};
use dorodoro_bangumi::{bencoding, hashmap, util, BoxWrapper};
    ...
    loop {
        ...
        if let RespType::Normal(msg_type, mut bytes) = resp_type {
            if msg_type == MsgType::Port {
                println!("port: {:?}", u32::from_be_slice(&bytes[0..4]))
...
impl CycleId {
    ...
    fn next_tran_id(&mut self) -> u16 {
        // self.val.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |x| {
        //     Some(if x == u16::MAX { 0 } else { x + 1 })
        // }).unwrap()
...
fn test_cycle_id() {
    ...
    for _ in 0..u16::MAX as usize + 10 {
        println!("next id: {}", cycle_id.next_tran_id());
    }
}

fn distancemetric(a: &[u8], b: &[u8]) -> [u8; 20] {
    if a.len() != 20 || b.len() != 20 {
        println!("长度不等\ta len: {}\tb len: {}", a.len(), b.len());
        panic!("长度不等");
    }
    let mut res = [0u8; 20];
    for i in 0..20 {
...
async fn test_peer_from_dht() {
    ...
    while !queue.is_empty() && peers.len() < 10 {
        ...
        if resp.is_err() {
            eprintln!("ping 的响应结果不符合预期\tping: {:?}", resp);
            continue;
        }
        let resp = resp.unwrap();
        if resp.r.is_none() {
            eprintln!("ping 的响应没有 r 字段: {:?}", resp);
            continue;
        }
        ...
        .await;
        if res.is_err() {
            eprintln!("读取 peers 响应超时");
            continue;
        }
        let res = res.unwrap();
        if res.is_err() {
            eprintln!("读取 peers 响应错误");
            continue;
        }
        ...
        if resp.is_err() {
            eprintln!("get_peers 的响应结果不符合预期\tget_peers: {:?}", resp);
            continue;
        }
...
#[tokio::test]
async fn test_dht_get_peers() {
    let req = [
        0x64u8, 0x31, 0x3a, 0x61, 0x64, 0x32, 0x3a, 0x69, 0x64, 0x32, 0x30, 0x3a, 0xd9, 0x82, 0x12,
        0xf0, 0xb1, 0xdb, 0x7, 0x46, 0x1a, 0x7e, 0x1f, 0x96, 0x50, 0x66, 0x42, 0x51, 0xe7, 0xe1,
        0x7d, 0x8d, 0x39, 0x3a, 0x69, 0x6e, 0x66, 0x6f, 0x5f, 0x68, 0x61, 0x73, 0x68, 0x32, 0x30,
        0x3a, 0xc6, 0xbb, 0xdb, 0x50, 0xbd, 0x68, 0x5b, 0xac, 0xf8, 0xc0, 0xd6, 0x15, 0xbb, 0x58,
        0xa3, 0xa0, 0x2, 0x39, 0x86, 0xef, 0x65, 0x31, 0x3a, 0x71, 0x39, 0x3a, 0x67, 0x65, 0x74,
        0x5f, 0x70, 0x65, 0x65, 0x72, 0x73, 0x31, 0x3a, 0x74, 0x32, 0x3a, 0xc7, 0xd0, 0x31, 0x3a,
        0x76, 0x34, 0x3a, 0x4c, 0x54, 0x2, 0xb, 0x31, 0x3a, 0x79, 0x31, 0x3a, 0x71, 0x65,
    ];
    let b = bencoding::decode(Bytes::from_owner(req.clone())).unwrap();
    println!("req: {:?}", b);

    let get_peers = DHTBase::<GetPeersReq>::request(
        GetPeersReq::new(
            Value::Bytes(Cow::from(vec![
                0xbd, 0xea, 0xcf, 0x53, 0x60, 0x64, 0xd9, 0x20, 0xf2, 0xa4, 0xe0, 0x4b, 0xa6, 0x86,
                0x51, 0x66, 0x3a, 0xfe, 0x9f, 0x66,
            ])),
            Value::Bytes(Cow::from(vec![
                0xbb, 0x59, 0xfe, 0x30, 0xba, 0x3b, 0xef, 0xe8, 0xcd, 0x14, 0x43, 0x37, 0x80, 0x5c,
                0x6b, 0x98, 0xb1, 0x41, 0x52, 0x57,
            ])),
        ),
        "get_peers".to_string(),
        0,
    ...
    let req = get_peers.to_bencode().unwrap();

    let socket = UdpSocket::bind("0.0.0.0:0").await.unwrap();
    let addr = "37.48.64.31:28015".parse::<SocketAddr>().unwrap();
    socket.send_to(&req, addr).await.unwrap();
    println!("成功发送 get peers");
...
#[test]
fn test() {
    let info_hash1 = hex::decode("c6bbdb50bd685bacf8c0d615bb58a3a0023986ef").unwrap();
    let info_hash2 = vec![
        0xc6u8, 0xbb, 0xdb, 0x50, 0xbd, 0x68, 0x5b, 0xac, 0xf8, 0xc0, 0xd6, 0x15, 0xbb, 0x58, 0xa3,
        0xa0, 0x2, 0x39, 0x86, 0xef,
    ];
    println!("info_hash1: {:?}", info_hash1);
    println!("info_hash2: {:?}", info_hash2);
    assert_eq!(info_hash1, info_hash2);
}

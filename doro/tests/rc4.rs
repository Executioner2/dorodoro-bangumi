...
use rc4::Rc4;
use rc4::{consts::*, KeyInit, Rc4Core, StreamCipher};
use sha1::{Digest, Sha1};
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tokio::net::TcpStream;
use tracing::{info, Level};
use doro::peer::reserved;
...
fn build_step3_packet(rc4: &mut Rc4Encrypt, s: &BigUint, skey: &[u8], vc: &[u8], crypto_provide: u32, pad_c: &[u8], ia: &[u8]) -> Vec<u8> {
    // 计算 req1
    let hash_req1 = {
        let mut hasher = Sha1::new();
    ...
    let hash_xor = {
        ...
        hash_req2.iter().zip(hash_req3.iter()).map(|(a, b)| a ^ b).collect::<Vec<_>>()
    };

...
struct VecHost(Vec<Host>);

impl Bytes2Object<VecHost> for [u8] {
    fn to_object(&self) -> Result<VecHost, Error> {
        let chunk_size = if self.len() % 26 == 0 {
            26
        } else if self.len() % 38 == 0 {
            38
        } else {
            return Err(Error::unexpected_token(
                "26 or 38 bytes per host expected",
                format!("Invalid host list length: {}", self.len()),
            ));
        };
        ...
        while let Some(pair) = dict.next_pair()? {
            match pair {
                ...
                (b"nodes", Object::Bytes(value)) => {
                    let vh: VecHost = value.to_object()?;
                    nodes = Some(vh.0);
                }
                (b"values", value) => {

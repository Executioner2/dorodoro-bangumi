...
pub trait FromBencode {
    ...
    /// Deserialize an object from its first byte representation.
    fn from_bencode_first(bytes: &[u8]) -> Result<(Self, usize), Error>
    where
        Self: Sized,
    {
        let mut decoder = Decoder::new(bytes)
            .with_max_depth(Self::EXPECTED_RECURSION_DEPTH);
